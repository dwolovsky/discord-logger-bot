...
const userHistoricalAnalysisData = new Map();
...
const dmFlowConfig = {
  ...
  'awaiting_outcome_suggestion_selection': {
    ...
    prompt: (setupData) => {
      ...
      const content = `Here are 5 outcome metric ideas.\n\nClick any one to EDIT it.`;
...
async function presentHistoricalMatchConfirmation(interaction, userId) {
    ...
    const row = new ActionRowBuilder().addComponents(includeButton, skipButton, finishButton);
...
/**
 * Sends the historical analysis report to the user as a series of separate messages.
 * @param {import('discord.js').Interaction} interaction - The interaction that triggered the analysis.
 * @param {object} report - The report data from the runHistoricalAnalysis Firebase function.
 */
async function sendHistoricalReport(interaction, report) {
    if (!report) {
        await interaction.editReply({ content: "No data was found for the selected metrics in that time period.", components: [] });
        return;
    }

    // Acknowledge the ephemeral "working" message is done.
    await interaction.editReply({
        content: `✅ Analysis complete! I've sent the report to you in new messages below.`,
        components: []
    });

    const user = interaction.user;
    const primaryMetricLabel = report.keyStats?.label || "your selected metric";

    try {
        // --- NEW: Page 1: AI Striking Insight ---
        if (report.strikingInsight) {
            const insightEmbed = new EmbedBuilder()
                .setColor('#FEE75C') // Yellow for insights
                .setTitle("💡 Your Big Insight")
                .setDescription(report.strikingInsight);
            await user.send({ embeds: [insightEmbed] });
        }

        // Page 2: Key Statistics
        if (report.keyStats) {
            const { label, average, median, consistency, dataPoints } = report.keyStats;
            const statsEmbed = new EmbedBuilder()
                .setColor('#0099FF')
                .setTitle(`Historical Analysis for '${label}'`)
                .setDescription(`Date Range: *Analysis currently includes all available data in your selection.*`)
                .addFields(
                    { name: 'Average', value: String(average), inline: true },
                    { name: 'Median', value: String(median), inline: true },
                    { name: 'Consistency', value: `${consistency.toFixed(1)}%`, inline: true },
                    { name: 'Data Points', value: String(dataPoints), inline: true }
                );
        ...
        if (report.correlations && report.correlations.length > 0) {
            ...
            const correlationEmbed = new EmbedBuilder()
                ...
                .setTitle(`Top ${report.correlations.length} Same-Day Correlations`)
                .setDescription(correlationDescription);
            await user.send({ embeds: [correlationEmbed] });
        }

        // Page 5: Combined Effects
        if (report.combinedEffects && report.combinedEffects.length > 0) {
            let comboDescription = `Here's how **'${primaryMetricLabel}'** interacts with other habits:\n\n`;

            report.combinedEffects.forEach(effect => {
            ...
            const comboEmbed = new EmbedBuilder()
                ...
                .setTitle('🔀 Combined Habit Effects')
                .setDescription(comboDescription);
            await user.send({ embeds: [comboEmbed] });
        }
        
        // Page 6: Lag Time Correlations
        if (report.lagCorrelations && report.lagCorrelations.length > 0) {
            let lagDescription = `Here are the strongest day-after relationships found for **'${primaryMetricLabel}'**:\n\n`;

            report.lagCorrelations.forEach(lag => {
                const rSquared = lag.coefficient * lag.coefficient;
                const absCoeff = Math.abs(lag.coefficient);
                let strengthText = "Weak";
                let strengthEmoji = "🟩";
            ...
            const lagEmbed = new EmbedBuilder()
                ...
                .setTitle('🕑 Day-After Effects')
                .setDescription(lagDescription);
            await user.send({ embeds: [lagEmbed] });
        }

        // Final completion message
        await user.send({ content: "This concludes your historical analysis report." });

    } catch (error) {
        console.error(`[sendHistoricalReport] Failed to send report pages to user ${user.id}:`, error);
        try {
            await interaction.followUp({
                content: "I couldn't send the report to your DMs. Please check your privacy settings to allow DMs from server members.",
                ephemeral: true
            });
        } catch (followUpError) {
             console.error(`[sendHistoricalReport] Failed to send fallback follow-up error.`, followUpError);
        }
    }
}
...
client.on(Events.InteractionCreate, async interaction => {
      ...
      try {
        ...
        switch (interaction.commandName) {
          ...
          case 'stats': {
            ...
            const interactionId = interaction.id;
            console.log(`[/stats START ${interactionId}] command invoked by ${interaction.user.tag}. Time: ${statsCommandStartTime.toFixed(2)}ms`);
            
            try {
                await interaction.deferReply({ flags: MessageFlags.Ephemeral });

                // 1. Fetch the user's current experiment settings
                const settingsResult = await callFirebaseFunction('getWeeklySettings', {}, interaction.user.id);
                if (!settingsResult || !settingsResult.settings) {
                    await interaction.editReply({ 
                        content: "You need an active experiment to analyze its history. Please set one up using the `/go` command.",
                        components: [] 
                    });
                    return;
                }
                ...
                // 2. Prepare the dropdown menu with the user's current metrics
                const metricSelectMenu = new StringSelectMenuBuilder()
                    .setCustomId('historical_metric_select')
                    .setPlaceholder('Select a metric to analyze...');

                const metrics = [settings.output, settings.input1, settings.input2, settings.input3]
                    .filter(metric => metric && metric.label); // Filter out any undefined/empty metrics

                if (metrics.length === 0) {
                     await interaction.editReply({ 
                        content: "Your current experiment doesn't seem to have any metrics defined. Please set one up using the `/go` command.",
                        components: [] 
                    });
                    return;
                }
                ...
                metrics.forEach(metric => {
                    metricSelectMenu.addOptions(
                        new StringSelectMenuOptionBuilder()
                            ...
                            .setValue(metric.label) // Using the label as the unique value
                    );
                });

                const row = new ActionRowBuilder().addComponents(metricSelectMenu);

                // 3. Store necessary data for the next step and set the state
                const analysisData = userHistoricalAnalysisData.get(interaction.user.id) || {};
                analysisData.historicalAnalysisData = { currentSettings: settings };
                analysisData.dmFlowState = 'awaiting_historical_metric_selection';
                userHistoricalAnalysisData.set(interaction.user.id, analysisData);

                // 4. Send the prompt with the dropdown menu
                await interaction.editReply({
                    content: "Select a metric from your current experiment to analyze its history:",
                    components: [row]
                });

                console.log(`[/stats SUCCESS ${interactionId}] Sent metric selection dropdown to ${interaction.user.tag}.`);

            } catch (error) {
                console.error(`[/stats ERROR ${interactionId}] Error processing /stats command for ${interaction.user.tag}:`, error);
                if (interaction.deferred || interaction.replied) {
                    await interaction.editReply({
                        content: `❌ An error occurred while fetching your experiment details: ${error.message || 'Please try again.'}`
                    });
                }
            }
            break;
  ...
  // --- Button Interaction Handler ---
    ...
    // --- Handler for Stats Report Navigation ---
        ...
        const interactionId = interaction.id;
        const userId = interaction.user.id;
        console.log(`[${interaction.customId} START ${interactionId}] User: ${userId} clicked Run Analysis.`);
   ...
   // Button handlers for the FINAL "Post to group?"
    ...
    console.log(`[ai_show_share_prompt_btn END ${interactionId}] Finished processing. Total time: ${(performance.now() - showSharePromptClickTime).toFixed(2)}ms`);
      ...
      try {
        ...
        const showModalStartTime = performance.now();
        const userId = interaction.user.id;
        const userTag = interaction.user.tag;
        const interactionId = interaction.id;
        console.log(`[${interaction.customId} START ${interactionId}] Clicked by ${userTag}.`);
        ...
        try {
            const setupData = userExperimentSetupData.get(userId);
            const chosenSuggestion = setupData?.tempSelectedOutcome;

            if (!chosenSuggestion) {
                console.error(`[${interaction.customId} ERROR ${interactionId}] tempSelectedOutcome not found for ${userTag}.`);
                await interaction.reply({ content: "Sorry, I've lost track of your selection. Please go back and choose your desired outcome again.", ephemeral: true });
                return;
            }

            // Create and show the modal, pre-populated with the AI's suggestion.
            const confirmOutcomeModal = new ModalBuilder()
                .setCustomId('confirm_ai_outcome_modal')
            ...
            console.error(`[${interaction.customId} ERROR ${interactionId}] Error showing modal for ${userTag}:`, error);
        }
    }
  ...
  // +++ COMPLETE MODAL SUBMISSION HANDLER FOR DAILY LOG (FIREBASE) +++
   // REPLACE THE EXISTING 'dailyLogModal_firebase' HANDLER
    ...
    try {
        ...
        // This finds the message that had the 'Edit' button and disables its components.
        if (setupData.messageToCleanUp) {
            try {
                const previousMessage = await interaction.channel.messages.fetch(setupData.messageToCleanUp);
                const disabledComponents = previousMessage.components.map(row =>
                    new ActionRowBuilder().addComponents(
                        row.components.map(component => ButtonBuilder.from(component).setDisabled(true))
                    )
                );
                await previousMessage.edit({ components: disabledComponents });
                console.log(`[${interaction.customId} CLEANUP_SUCCESS ${interactionId}] Disabled buttons on previous message ${setupData.messageToCleanUp}.`);
            } catch (e) {
                if (e.code !== 10008) { // Ignore "Unknown Message" error, but log others
                    console.warn(`[${interaction.customId} CLEANUP_FAIL ${interactionId}] Could not disable buttons on previous message ${setupData.messageToCleanUp}: ${e.message}`);
                }
            }
            delete setupData.messageToCleanUp;
        }
        // --- END UPDATED CLEANUP LOGIC ---
        ...
        */
        ...
        // --- UPDATED DATA INTEGRITY FIX ---
        // This line now correctly resets the habits only when starting from scratch.
        if (setupData.setupStyle === 'scratch' || !setupData.inputs) {
            setupData.inputs = [];
        }
        ...
        try {
            ...
            if (!setupData || !setupData.outcome) {
                console.error(`[${interaction.customId} CRITICAL ${interactionId}] In-memory setup state missing outcome for user ${userTag}.`);
                await interaction.editReply({ content: '❌ Error: Your setup session has expired or is invalid. Please restart the setup.', components: [], embeds: [] });
                return;
            }
            ...
            const validationErrors = [];
            if (!habit1Label) {
                validationErrors.push("The 'Habit 1' label is required.");
            } else if (habit1Label.length > 30) {
                validationErrors.push(`The Habit 1 Label is too long (max 30 characters).`);
            }

            if (!habit1Unit) {
                validationErrors.push("The 'Unit / Scale' for Habit 1 is required.");
            } else if (habit1Unit.length > 15) {
                validationErrors.push(`The Unit/Scale for Habit 1 is too long (max 15 characters).`);
            }
            
            if (habit1Label && habit1Unit && (habit1Label.length + habit1Unit.length + 1) > 45) {
                validationErrors.push(`The combined Label and Unit for Habit 1 are too long for the daily log form (max 45 chars).`);
            }
        ...
        try {
            ...
            const validationErrors = [];
            if (!habit2Label) {
                validationErrors.push("The 'Habit 2' label is required.");
            } else if (habit2Label.length > 30) {
                validationErrors.push(`The Habit 2 Label is too long (max 30 characters).`);
            }

            if (!habit2Unit) {
                validationErrors.push("The 'Unit / Scale' for Habit 2 is required.");
            } else if (habit2Unit.length > 15) {
                validationErrors.push(`The Unit/Scale for Habit 2 is too long (max 15 characters).`);
            }

            if (habit2Label && habit2Unit && (habit2Label.length + habit2Unit.length + 1) > 45) {
                validationErrors.push(`The combined Label and Unit for Habit 2 are too long for the daily log form (max 45 chars).`);
            }
            ...
            if (!habit2GoalStr) {
                ...
                const goalResult = parseGoalValue(habit2GoalStr);
                if (goalResult.error) {
                    validationErrors.push(goalResult.error);
                } else {
                    habit2Goal = goalResult.goal;
                }
            }
            ...
            */
            ...
            const confirmationEmbed = new EmbedBuilder()
                .setColor('#57F287')
                .setTitle('✅ Habit 2 Confirmed!')
                .setDescription(`**${formatGoalForDisplay(habit2Goal, habit2Unit)} ${habit2Unit}, ${habit2Label}**`)
                .addFields({ name: '\u200B', value: "Would you like to add a 3rd (and final) habit to test?" });
        ...
        try {
            ...
            const validationErrors = [];
            if (!habit3Label) {
                validationErrors.push("The 'Habit 3' label is required.");
            } else if (habit3Label.length > 30) {
                validationErrors.push(`The Habit 3 Label is too long (max 30 characters).`);
            }

            if (!habit3Unit) {
                validationErrors.push("The 'Unit / Scale' for Habit 3 is required.");
            } else if (habit3Unit.length > 15) {
                validationErrors.push(`The Unit/Scale for Habit 3 is too long (max 15 characters).`);
            }

            if (habit3Label && habit3Unit && (habit3Label.length + habit3Unit.length + 1) > 45) {
                validationErrors.push(`The combined Label and Unit for Habit 3 are too long for the daily log form (max 45 chars).`);
            }
            ...
            */
            ...
            const fullSummaryEmbed = new EmbedBuilder()
                ...
                .setDescription(
                    `Here is the full summary of your experiment. If it looks correct, click "Finish Setup" to proceed. You can also go back to edit the Outcome.\n\n` +
                    `**Deeper Wish:**\n${setupData.deeperProblem}\n\n` +
