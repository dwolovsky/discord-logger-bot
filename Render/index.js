...
// Add this with your other Map definitions
const userHistoricalReportData = new Map();

// Add these with your other Custom IDs
const HISTORICAL_REPORT_MORE_STATS = 'historical_report_more_stats';
const HISTORICAL_REPORT_SHOW_SHARE = 'historical_report_show_share';
const HISTORICAL_REPORT_NO_SHARE = 'historical_report_no_share';
const HISTORICAL_REPORT_POST_PUBLIC = 'historical_report_post_public';
...
async function sendNextTimeLogPrompt(interaction, userId) {
  ...
  if (timeLogIndex < logFlowTimeMetrics.length) {
    ...
    const finalButton = new ButtonBuilder()
        ...
        .setLabel('Continue to log notes')
        .setStyle(ButtonStyle.Success);
...
// --- New Custom IDs for Historical Stats Pagination ---
const HISTORICAL_NAV_BACK_ID = 'historical_nav_back';
const HISTORICAL_NAV_NEXT_ID = 'historical_nav_next';
    ...
    try {
               if (part === 'aha_moment') {
                ...
                if (report.hiddenGrowth) {
                    ...
                    // Ideal Case: It's an object with both quote and paragraph
                    ...
                    // Fallback Case: It's just a single string
            ...
            // --- DM #5: Lag Time Correlations (Descriptive) ---
            ...
            report.analyzedChapters.forEach(chapter => {
                ...
                const relevantLagCorrsText = lagCorrs.filter(lag => lag.todayMetricLabel === primaryLabel)
                    .map(lag => {
                        const absCoeff = Math.abs(lag.coefficient);
                        ...
                        const todayUnit = unitMap[lag.todayMetricLabel];
                        const isYesterdayTime = isTimeMetric(yesterdayUnit);
                        const isTodayTime = isTimeMetric(todayUnit);
  ...
  // --- Button Interaction Handler ---
    ...
    try {
        ...
        if (cachedSettings) {
            ...
            if (hasTimeMetrics) {
                console.log(`[log_daily_progress_btn CACHE_TIME_METRICS ${interactionId}] Path A: Cached settings have time metrics. Deferring update.`);
                await interaction.deferUpdate();
                ...
                const modal = buildStandardLogModal(cachedSettings, setupData);
                await interaction.showModal(modal);
                // Clear the temp data after using it
                if (setupData.tempLogData) {
                    delete setupData.tempLogData;
                    userExperimentSetupData.set(userId, setupData);
                }
            }
        } 
    ...
    try {
        ...
        const modal = new ModalBuilder().setCustomId('dailyLogModal_firebase').setTitle(`📝 Notes & Other Metrics`);
        const components = [];
        const temp_notes = setupData.tempLogData?.notes || "";
        const temp_output = setupData.tempLogData?.outputValue || "";
        const temp_inputs = setupData.tempLogData?.inputValues || [];
        ...
        // --- NEW: Clear the temp data after using it ---
        if (setupData.tempLogData) {
            delete setupData.tempLogData;
            userExperimentSetupData.set(userId, setupData);
            logger.log(`[${interaction.customId}] Cleared temporary log data for ${userId} after pre-populating modal.`);
        }
    ...
    try {
        ...
        await interaction.update({ content: "👍 Understood. Your report is complete and remains private.", components: [], embeds: [] });
        userHistoricalReportData.delete(interaction.user.id); // Clean up session
    }

    else if (interaction.customId === HISTORICAL_REPORT_SHOW_SHARE) {
        try {
            await interaction.deferUpdate();
    ...
    else if (interaction.customId === HISTORICAL_REPORT_POST_PUBLIC) {
        try {
            await interaction.deferUpdate();
            const reportData = userHistoricalReportData.get(interaction.user.id);
            ...
            if (channel && channel.isTextBased()) {
             ...
             console.error(`Error posting public report:`, error);
             await interaction.editReply({ content: `An error occurred while posting: ${error.message}`, components: [] });
        } finally {
            userHistoricalReportData.delete(interaction.user.id); // Clean up session
        }
    }
   ...
   // Button handlers for the FINAL "Post to group?"
    ...
    try {
        ...
        await interaction.editReply({ content: `✅ Found **${includedMetrics.length}** exact match(es) for "${selectedLabel}" in your history. Now, I'm checking for other potential matches...`, embeds: [], components: [] });
   ...
   // REPLACE THE EXISTING 'dailyLogModal_firebase' HANDLER
    if (interaction.isModalSubmit() && interaction.customId === 'dailyLogModal_firebase') {
        ...
        try {
            ...
            logger.log(`[dailyLogModal_firebase] Saved temporary log data for ${interaction.user.tag} due to validation failure.`);
            
            await interaction.editReply({ content: "❌ Missing required fields (Outcome, Habit 1, or Notes). When you reopen the form, your notes will be preserved." });
            return;
        }
            ...
            // --- NEW: Clean up any temporary log data on successful submission ---
            if (setupData && setupData.tempLogData) {
                delete setupData.tempLogData;
                userExperimentSetupData.set(interaction.user.id, setupData);
                logger.log(`[dailyLogModal_firebase] Cleared temporary log data for ${interaction.user.tag} after successful submission.`);
            }
